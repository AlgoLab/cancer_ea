""" script contente funzione per la generazione della matrice inferita
    in un file txt e per la generazione del formato graph.
"""
...
#       print(dic_mut)
        ...
        for pre,_,node in RenderTree(tree):
                list_tmp1.append(node.node_label)
                tmp1=list_tmp1.count(node.node_label)-1
                if node.node_label in index_node.keys():
                        index_node[node.node_label+str(tmp1-1)]=cont1

                else:
                        index_node[node.node_label]=cont1
                cont1+=1
        """
                        ...
                        if node.node_label in index_parent.keys():
                                ...
                                for child in node.children:
                                        if child.node_label in list_tmp:
                                                ...
                                                node_children.append(child.node_label+str(tmp))

                                        elif child.node_label in node_children:
                                                l.append(child.node_label+str(tmp-1))
                                                node_children.append(child.node_label+str(tmp-1))
                                        else:
                                                l.append(child.node_label)
                                                node_children.append(child.node_label)
        ...
        for key in index_parent.keys():
                for val in index_parent[key]:
                        file.write("    "+'"'+str(dic_enum[key])+'" -> "'+str(dic_enum[val])+'";\n')
